...
class BaseModel extends Model
{
    ...
    {
        if (is_null($this->paginate)) return $oQuery;
    ...
    public function exists($iPrimaryKeyOraWhere)
    {
        ...
        if ($result) return $result->{$this->primaryKey};
     ...
     * @return bool
     */
    public function baseUpdate(array $aData, $iPrimaryKeyOraWhere)
    {
        if (count($aData) < 1) return FALSE;
     ...
     * @return int
     */
    public function baseDelete(array $aWhere)
    {
    ...
    public function fetchRow($iPrimaryKeyOraWhere)
    {
        switch (gettype($iPrimaryKeyOraWhere)) {
            ...
            default:
                ...
                $result = DB::select('select ' . $select_items . ' from ' . $this->table . ' where ' . $this->primaryKey . ' = ? '.$groupBy, array((int)$iPrimaryKeyOraWhere));
                break;
        }
